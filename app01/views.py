...
def task_01(request):
    ...
    eng.cd('E:\MatlabWorkPlace\参考1_BELLHOP_General_Description\Task_GD_1_flat', nargout=0)

    # 执行 MATLAB 脚本文件
    eng.eval("run('command_draft_Task1_Munk_flat.m');", nargout=0)
    # 循环获取当前图形句柄
    # num = 3
    # while(num>0):
    ...
    # 将图像数据传递到模板
    return render(request, 'task_01.html', {'image_data': image_base64})
def task_02(request):
...
def task_03(request):
    # 启动 MATLAB 引擎
    eng = matlab.engine.start_matlab()
    eng.cd('E:\MatlabWorkPlace\参考1_BELLHOP_General_Description\Task_GD_3_VB', nargout=0)

    # 执行 MATLAB 脚本文件
    eng.eval("run('command_draft_Task3_VariousBounds.m');", nargout=0)
    # 获取当前图形句柄
    fig_handle = eng.gcf()
    ...
    # 将图像数据传递到模板
    return render(request, 'task_03.html', {'image_data': image_base64})
def Page02(request):
    #获取请求方式 GET/POST
